...
def creat_matrix_sideweight(road_information):
    a1 = np.zeros((len(road_information) - 1, len(road_information) - 1), dtype=int)

    for i in range(1, len(road_information)):
        k = eval(road_information[i])
        length = int(k[1])
        start_point = int(k[4])
        final_point = int(k[5])
        a1[start_point - 1][final_point - 1] = length
        if int(k[6]) != 1:
            a1[final_point - 1][start_point - 1] = 0
        else:
            a1[final_point - 1][start_point - 1] = length

    return a1

def creat_orientational_matrix(cross_information, road_information):
    a2 = np.zeros((len(cross_information) - 1, len(cross_information) - 1), dtype=int)
    for i in range(1, len(cross_information)):
        item = eval(cross_information[i])
        for k in range(1, 5):
            if item[k] != -1:
                number = int(item[k] % 5000 + 1)
                road = eval(road_information[number])
                start_point = road[4]
                final_point = road[5]
                if k == 1:
                    a2[start_point - 1][final_point - 1] = 1
                    a2[final_point - 1][start_point - 1] = 3
                if k == 2:
                    a2[start_point - 1][final_point - 1] = 2
                    a2[final_point - 1][start_point - 1] = 4
    return a2

def orientational_x_y_dict(cross_information, road_information):
    m = []
    point_site = {}
    flag = np.zeros((len(cross_information) - 1, 1), dtype=int)
    close_flag = np.zeros((len(cross_information) - 1, 1), dtype=int)
    point_site['node_num{}'.format(1)] = (0, 0)
    # print(point_site['node_num{}'.format(1)])
    close_flag[0] = 1
    m.append(eval(cross_information[1]))

    while len(m) != 0:

        first_item = m.pop(0)

        flag[first_item[0] - 1] = 1
        for i in range(1, 5):
            slect_road = first_item[i]
            if slect_road != -1:
                index = int(slect_road % 5000 + 1)
                point1 = eval(road_information[index])[4]
                point2 = eval(road_information[index])[5]
                if point1 == first_item[0]:
                    if flag[point2 - 1] == 0:
                        m.append(eval(cross_information[point2]))
                        number = eval(cross_information[point2])[0]
                        x = point_site['node_num{}'.format(first_item[0])][0]
                        y = point_site['node_num{}'.format(first_item[0])][1]
                        road_lenght = int(eval(road_information[index])[1])
                        if i == 1:
                            if close_flag[number - 1] == 0:
                                point_site['node_num{}'.format(number)] = (x, y + road_lenght)
                                close_flag[number - 1] = 1
                        if i == 2:
                            if close_flag[number - 1] == 0:
                                point_site['node_num{}'.format(number)] = (x + road_lenght, y)
                                close_flag[number - 1] = 1
                        if i == 3:
                            if close_flag[number - 1] == 0:
                                point_site['node_num{}'.format(number)] = (x, y - road_lenght)
                    ...
                    if flag[point1 - 1] == 0:
                        m.append(eval(cross_information[point1]))
                        number = eval(cross_information[point1])[0]
                        x = point_site['node_num{}'.format(first_item[0])][0]
                        y = point_site['node_num{}'.format(first_item[0])][1]
                        if i == 1:
                            if close_flag[number - 1] == 0:
...
class AStar(object):
    ...
    def __init__(self, cross_information, road_information, array_orientational, array_sideweight, dict_site, car):
        """
        初始化
        """
        # self.g = 0  # g初始化为0
        ...
        self.cross_information = cross_information
        self.road_information = road_information
        self.array_orientational = array_orientational
        self.array_sideweight = array_sideweight
        self.dictionary_position = dict_site
        self.car = car
    ...
    def h_value_tem(self, son_point):
        ...
        h = (self.dictionary_position['node_num{}'.format(son_point[0])][0] -
             self.dictionary_position['node_num{}'.format(self.goal_point)][0]) ** 2 + (
                        self.dictionary_position['node_num{}'.format(son_point[0])][1] -
                        self.dictionary_position['node_num{}'.format(self.goal_point)][1]) ** 2
        h = np.sqrt(h)  # 计算h
        return h

    def g_accumulation(self, son_point, father_point):
        item_father = eval(self.cross_information[father_point[0]])  # 读取父、子节点信息
        item_son = eval(self.cross_information[son_point[0]])
        for i in range(1, 5):
            if item_father[i] != -1:
                for j in range(1, 5):
                    if item_father[i] == item_son[j]:
                        g1 = eval(self.road_information[item_father[i] % 5000 + 1])[1]
                        break
    ...
    def child_point(self, father_point):
        ...
        for i in range(1, 5):
            ...
            father_point_orientation_list = self.array_orientational[father_point_ID - 1]
            # print(father_point_orientation_list)
            if i in father_point_orientation_list:
                # pdb.set_trace();
                k_index = np.where(father_point_orientation_list == i)[0][0]
                child_cross_ID = k_index + 1

                road_id_two_point = eval(self.cross_information[father_point_ID])[i]                       #找到子父节点间的道路信息
                road_information_two_point = eval(self.road_information[road_id_two_point%5000+1])
                if road_information_two_point[6] ==0:
                    if road_information_two_point[4]== father_point_ID:
                        child_cross_point = [child_cross_ID, i, 0, 0,0]
                    ...
                    child_cross_point = [child_cross_ID, i, 0, 0]
    ...
    def main(self):
        ...
        while ite <= 1000:
            ...
            # return  car_path
#生成车辆已出发信息列表
def creat_run_car_dict(list_car_time,time,path_dict):
    run_car_dict = {}

    while not run_car_dict:  # 这个判断有错
        print(time)
        while eval(list_car_time[0])[4] == time:
            ...
            if len(path_dict[item]) <= 2:
                cross_id_information = eval(cross_information[start_cross_id])
                road_position_id = cross_id_information[car_orientation_1]
                run_car_dict['{}'.format(eval(list_car_time[0])[0])] = [go_straight, car_speed, wait_select,road_position_id]
                run_car_dict.update()
                list_car_time.pop(0)
                ...
                if car_orientation_1 == 1 and car_orientation_2 == 2:
                    cross_id_information = eval(cross_information[start_cross_id])
                    road_position_id = cross_id_information[car_orientation_1]
                    run_car_dict['{}'.format(eval(list_car_time[0])[0])] = [turn_right, car_speed, wait_select,road_position_id]
                    list_car_time.pop(0)
                    if len(list_car_time) == 0:
                        break
                    else:
                ...
                if car_orientation_1 == 1 and car_orientation_2 == 4:
                    cross_id_information = eval(cross_information[start_cross_id])
                    road_position_id = cross_id_information[car_orientation_1]
                    run_car_dict['{}'.format(eval(list_car_time[0])[0])] = [turn_left, car_speed, wait_select,road_position_id]
                    list_car_time.pop(0)
                    if len(list_car_time) == 0:
                        break
                    else:
                ...
                if car_orientation_1 == 2 and car_orientation_2 == 1:
                    road_position_id = cross_information[start_cross_id][car_orientation_1]
                    run_car_dict['{}'.format(eval(list_car_time[0])[0])] = [turn_left, car_speed, wait_select,road_position_id]
                    list_car_time.pop(0)
                    if len(list_car_time) == 0:
                        break
...
road_information = read_file('road.txt')                         #生成地图模型、车静态信息
cross_information = read_file('cross.txt')
array_orientational = creat_orientational_matrix(cross_information, road_information)
array_sideweight = creat_matrix_sideweight(road_information)
dict_site = orientational_x_y_dict(cross_information, road_information)
car_information = read_file('car.txt')
path_dict = {}
print(car_information[0])



road_dict ={}                                        #生成道路模型
for i in range(1,len(road_information)):
    road_information_1 = eval(road_information[i])
    road_model = road(road_information_1)
    road_model=road_model.main_creat_road()

    road_dict['{}'.format(road_information_1[0])]=road_model

for i in range(1,len(car_information)):                                #生成车辆行驶路径
    car_in_all = eval(car_information[i])
    k = AStar(cross_information, road_information, array_orientational, array_sideweight, dict_site, car_in_all)
    path = k.main()
    path_dict['{}'.format(car_in_all[0])] = path

#生成车辆时间排序表
list_car_time_1 = car_information[:]
list_car_time_1.pop(0)
list_car_time = sorted(list_car_time_1,key=lambda list_car_time_1:eval(list_car_time_1)[4])

run_car_dict,time = creat_run_car_dict(list_car_time,time,path_dict)            #初始化车辆出发表
...
for i in range(len(run_car_list)):                                            #遍历run_car_dict,将车辆安排上路
    car_initial_id=int(run_car_list[i][0])
    test=car_initial_id%10000
    car_initial_information =eval(car_information[car_initial_id%10000+1])           #获取派车上路所需信息
    ...
    car_start_road =eval(cross_information[start_point])[car_initial_orientation]
    road_model=road_dict['{}'.format(car_start_road)]
    car_road_information = eval(road_information[car_start_road%5000+1])
...
#第一次将车派上路
'''
for i in run_car_dict.keys():                                            #遍历run_car_dict,将车辆安排上路
    car_initial_id=int(format(i))
    test=car_initial_id%10000
    car_initial_information =eval(car_information[car_initial_id%10000+1])           #获取派车上路所需信息
    start_point = car_initial_information[1]
    car_initial_speed = car_initial_information[3]
    ...
    if road_model[0,0] == start_point:                             #如果所派车辆，所在道路为正向道路
        ...
        if flag_no_free_channel ==1:                                             #若车道无空位，车辆信息重新进入list_car_time
            run_car_dict.pop('{}'.format(car_initial_id))
            car_initial_information_list=[car_initial_information]
            list_car_time =  car_initial_information_list + list_car_time
            list_car_time[0][4]= time
...
def conflict_judgment(j,k,l,car_orientation_state,cross_model,run_car_dict,road_dict):  #通过当前车辆信息，查询是否其他道路有跟这辆车起冲突的车
    ...
    road_model_information = eval(road_information[cross_road_id % 5000 + 1])
...
def go_straigh_car_conflict_find(cross_model,conflict_road_id,run_car_dict,road_dict): #检测这条路第一优先级是否有直行并且能过路口的车辆（冲突车辆），给予路口以及道路信息
    ...
    conflict_road_information = eval(road_information[conflict_road_id % 5000 + 1])
        ...
        if conflict_road_model[0, conflict_road_length + 1] == cross_model_id:  # 单行道的话，看这条路是不是通往路口，是的话，该条道路可能存在冲突车
            for channels in range(conflict_road_width):
                ...
                for index in range(conflict_road_length, 0, -1):
                    if conflict_road_model[channels, conflict_road_length] != 0:  # 假如这个位置存在车辆的话，则是第一优先级车辆
                        car_in_conflict_road_id = conflict_road_model[channels, conflict_road_length]
        ...
        if conflict_road_model[0, conflict_road_length + 1] == cross_model_id:  # 双行道，则此时用的上半部分路
            for channels in range(conflict_road_width):
                ...
                for index in range(conflict_road_length, 0, -1):
                    if conflict_road_model[channels, conflict_road_length] != 0:  # 假如这个位置存在车辆的话，则是第一优先级车辆
                        car_in_conflict_road_id = conflict_road_model[channels, conflict_road_length]
                        car_in_conflict_road_information = run_car_dict[
                            '{}'.format(car_in_conflict_road_id)]
                        car_in_conflict_road_speed = car_in_conflict_road_information[1]
                        if run_car_dict['{}'.format(car_in_conflict_road_id)][2] == 2:  # 如果这辆车已经终止，则不管了
                            break
                        else:  # 这两车是等待的情况
...
def car_run_next_road(j,k,l,car_orientation_state,cross_model,car_num_waitting,car_num_arrived,run_car_dict,road_dict,up_or_down_flag,car_num_stopped):
    ...
    car_end_point = eval(car_information[item_car_in_road_id % 10000 + 1])[2]
    ...
    road_model_next_information = eval(road_information[road_model_next_id % 5000 + 1])
    ...
    if road_model_next[0, 0] == cross_model_id:  # 判断要进入的这条路的道路矩阵，看是上半部分还是下半部分，if成立则是上半部分
        ...
        if flag_crowded == 1:  # 没有空位，看下堵塞的车辆是不是全为停止状态，若是，则当前车辆也停止，否则继续等待
            ...
            for present_road_width in range(road_model_next_wight):  # 看要进入的下一路有没有空位能进入
                if run_car_dict['{}'.format(road_model_next[present_road_width, 1])][2]==2:  # 找到一个空位则退出，当前空位为第一优先级进入空位
...
time = time +1
while car_num_arrived !=(len(car_information)-1):
    ...
    for i in range(1, len(road_information)):  # 遍历所有道路模型
        ...
        for j in range(0, road_width):  # 遍历正向道路
            ...
            for k in range(road_length ,0,-1):                  #从最前面的车开始遍历,road_length+1报错
                ...
                if road_model[j, k] !=0:
                    ...
                    item_car_in_road_speed =eval(car_information[item_car_in_road_id%10000+1])[3]
    ...
    while car_num_waitting !=0:                      #再次以节点为顺序，遍历道路
        ...
        for i in range(1,len(cross_information)):
            if car_num_waitting == 0:
                break
            cross_model = eval(cross_information[i])
            cross_model_id = cross_model[0]
            for j in range (1,5):
                if car_num_waitting == 0:
                    break
